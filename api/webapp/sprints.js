...
const { getActiveSprints } = require('../../src/database/queries/sprints');
...
module.exports = async (req, res) => {
  ...
  const { data: sprintRows } = await getActiveSprints(user.id);
  if (!sprintRows || sprintRows.length === 0) return res.status(200).json({ sprints: [] });

  const today = new Date();

  const sprints = await Promise.all(sprintRows.map(async (sprint) => {
